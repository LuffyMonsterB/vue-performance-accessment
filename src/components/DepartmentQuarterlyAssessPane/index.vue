<template>
  <div>
    <el-card>
      ...
      <div style="margin-bottom:1em;">
        <b style="font-size:18px">部门季度绩效考核</b>
      </div>
      <el-table
        v-loading="tableLoading"
        :data="departmentQuarterlyAssessTable"
        border
        fit
      ...
      >
        ...
        <el-table-column label="季度绩效考核得分" align="center">
          <template slot-scope="{ row }">
            <el-tag>{{ parseInt(row.keyWork) + parseInt(row.basicWork) + parseInt(row.commonWork) }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog title="绩效修改" :visible.sync="editFormVisible" width="30%">
      <el-form
        ref="dataForm"
        :rules="editFormRules"
        :model="tempEditForm"
...
<script>
import { genDepartmentQuarterAssessData,queryDepartmentQuarterAssessDataByDepartment } from '@/api/department-assess'
import waves from '@/directive/waves'
import Pagination from '@/components/Pagination'

export default {
  name: 'DepartmentQuarterlyAssessPane',
  directives: { waves },
  ...
  data() {
    return {
      departmentQuarterlyAssessTable: null,
      tableTitle: '',
      tableQuery: {
        page: 1,
        limit: 20,
      ...
      filtering:false,
      downloading:false,
      departmentOptions:["办公室", "财务科", "专卖科", "稽查中队", "城关专卖所","力洋专卖所", "梅林专卖所","业务科","中转站","城关市场部","力洋市场部","梅林市场部"]
    }
  },
  created() {
    genDepartmentQuarterAssessData().then(res => {
      this.departmentQuarterlyAssessTable =
        res.data.items.department_quarterly_assess_table.department_assess
    })
  },
  methods: {
    handleFilter() {
      if (this.tableQuery.department) {
        this.filtering = true;
        setTimeout(() => {
          queryDepartmentQuarterAssessDataByDepartment(this.tableQuery.department).then(res => {
            if(res.data.items.query){
              this.departmentQuarterlyAssessTable=[res.data.items.query]
    ...
    handleEdit(index) {
      this.tempEditForm = this.departmentQuarterlyAssessTable[index]
